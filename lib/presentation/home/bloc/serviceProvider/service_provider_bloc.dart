...
class ServiceProviderBloc
  ...
  ServiceProviderBloc({
    ...
    on<FetchAllServiceProvidersWithoutLocation>(_onFetchAllWithoutLocation);
  }
  ...
  Future<void> _onFetchAllWithoutLocation(
    FetchAllServiceProvidersWithoutLocation event,
    Emitter<ServiceProviderState> emit,
  ) async {
    emit(ServiceProviderLoading());
    try {
  ...
  Future<void> _onFetchAll(
    ...
    try {
      final stream = getAllServiceProviders();

      await for (final providers in stream) {
        final enriched = await DistanceMatrixHelper.addDistanceAndTime(
          userLat: event.latitude,
          userLng: event.longitude,
          providers: providers,
        );
        emit(ServiceProviderLoaded(enriched));
      }
    } catch (e) {
      emit( ServiceProviderError('Something went wrong ${e}'));
    }
  }
  ...
  Future<void> _onFetchNearby(
    ...
    emit(ServiceProviderLoading());
    try {
      final providersStream = getNearbyServiceProviders(
        event.latitude,
        event.longitude,
      );

      await for (final providers in providersStream) {
        final enriched = await DistanceMatrixHelper.addDistanceAndTime(
          userLat: event.latitude,
          userLng: event.longitude,
          providers: providers,
